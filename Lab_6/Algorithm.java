...
public class Algorithm {
    ...
    public Algorithm(int numberOfNodes) {
    ...
    public void johnsonsAlgorithms(int adjacencyMatrix[][]) {
        ...
        for (int source = 1; source <= numberOfNodes; source++) {
            ...
            for (int destination = 1; destination <= numberOfNodes; destination++) {
                allPairShortestPath[source][destination] = result[destination] + potential[destination]
                        - potential[source];
            }
        }

        System.out.println();
        System.out.println("матрица всех кратчайших путей:");
    ...
    private int[][] reweightGraph(int adjacencyMatrix[][]) {
        ...
        for (int source = 1; source <= numberOfNodes; source++) {
            for (int destination = 1; destination <= numberOfNodes; destination++) {
                result[source][destination] = adjacencyMatrix[source][destination] + potential[source]
    ...
    public static void main(String... arg) {
        ...
        try {
            ...
            System.out.println("Введите взвешенную матрицу для графика");
            ...
            Algorithm johnsonsAlgorithm = new Algorithm(number_of_vertices);
            johnsonsAlgorithm.johnsonsAlgorithms(adjacency_matrix);
            long end = System.nanoTime();
            long duration = end - start;
            System.out.println("Время выполнения: " + (duration / 1000000L) + " мс");
        ...
        for (int sourcenode = 1; sourcenode <= numberofvertices; sourcenode++) {
            for (int destinationnode = 1; destinationnode <= numberofvertices; destinationnode++) {
                if (adjacencymatrix[sourcenode][destinationnode] != MAX_VALUE) {
                    if (distances[destinationnode] > distances[sourcenode]
                            + adjacencymatrix[sourcenode][destinationnode])
                        System.out.println("Граф содержит отрицательные вершины");
                }
            }
        }
    }
...
class DijkstraShortesPath {
    ...
    public void dijkstraShortestPath(int source, int adjacencymatrix[][]) {
        ...
        this.unvisited = new boolean[numberofvertices + 1];
        ...
        distances[source] = 0;
        while (getUnsettledCount(unvisited) != 0) {
            evaluationnode = getNodeWithMinimumDistanceFromUnsettled(unvisited);
            unvisited[evaluationnode] = false;
            visited[evaluationnode] = true;
        ...
        for (int destinationNode = 1; destinationNode <= numberofvertices; destinationNode++) {
            if (visited[destinationNode] == false) {
                if (adjacencymatrix[evaluationNode][destinationNode] != MAX_VALUE) {
                    ...
                    unvisited[destinationNode] = true;
                }
            }
        }
    }
